...
export default defineComponent({
  ...
  data() {
    return {
      comments: [],
      stanceColor: stanceColors["Sympathy_Memorial_Commemorative"] || "#f0f0f0",
      scrollInterval: null,
  ...
  methods: {
        ...
        complete: (results) => {
          this.comments = results.data
            .filter((row) => {
              ...
              return stance === "sympathy_memorial_commemorative";
            })
            .map((row) => {
              ...
              cleanedText = this.addSpacingBetweenSentences(cleanedText);

              return cleanedText;
            });

          console.log("Filtered sympath,memorial comments:", this.comments);
          this.$nextTick(() => {
            this.startScrolling();
          });
        },
    ...
    addSpacingBetweenSentences(text) {
      return text
        .replace(/([.!?])([A-Z])/g, "$1 $2")
        .replace(/suffering/gi, "<b>$&</b>")
        .replace(/remember/gi, "<b>$&</b>")
        .replace(/tragedy/gi, "<b>$&</b>")
        .replace(/suffer/gi, "<b>$&</b>")
        .replace(/horrible/gi, "<b>$&</b>")
        .replace(/never forget/gi, "<b>$&</b>")
        .replace(/story/gi, "<b>$&</b>")
        .replace(/tragic/gi, "<b>$&</b>")
        .replace(/pain/gi, "<b>$&</b>")

        .replace(/forgotten/gi, "<b>$&</b>")
        .replace(/suffered/gi, "<b>$&</b>");
    },
  },
});
</script>
